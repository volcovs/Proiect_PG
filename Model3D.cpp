...
namespace gps {
	...
	void Model3D::LoadModel(std::string fileName, float fragElim) {

		std::string basePath = fileName.substr(0, fileName.find_last_of('/')) + "/";
		ReadOBJ(fileName, basePath, fragElim);
	}

	void Model3D::LoadModel(std::string fileName, std::string basePath, float fragElim) {
	...
	void Model3D::ReadOBJ(std::string fileName, std::string basePath, float fragElim) {
			...
			if (a > 0 && materials.size() > 0) {
				...
				if (materialId != -1) {
					...
					if (!ambientTexturePath.empty()) {
						...
						currentTexture = LoadTexture(basePath + ambientTexturePath, "ambientTexture", fragElim);
						textures.push_back(currentTexture);
					}
					...
					if (!diffuseTexturePath.empty()) {
						...
						gps::Texture currentTexture;
						currentTexture = LoadTexture(basePath + diffuseTexturePath, "diffuseTexture", fragElim);
						textures.push_back(currentTexture);
					}
					...
					if (!specularTexturePath.empty()) {
						...
						gps::Texture currentTexture;
						currentTexture = LoadTexture(basePath + specularTexturePath, "specularTexture", fragElim);
						textures.push_back(currentTexture);
					}
				}
			}
	...
	gps::Texture Model3D::LoadTexture(std::string path, std::string type, float fragElim) {
		...
		gps::Texture currentTexture;
		currentTexture.id = ReadTextureFromFile(path.c_str(), fragElim);
	...
	GLuint Model3D::ReadTextureFromFile(const char* file_name, float fragElim) {
