...
namespace gps {
	...
	public:
		...
		void LoadModel(std::string fileName, float fragElim);

		void LoadModel(std::string fileName, std::string basePath, float fragElim);
	...
	private:
		...
		void ReadOBJ(std::string fileName, std::string basePath, float fragElim);

		// Retrieves a texture associated with the object - by its name and type
		gps::Texture LoadTexture(std::string path, std::string type, float fragElim);

		// Reads the pixel data from an image file and loads it into the video memory
		GLuint ReadTextureFromFile(const char* file_name, float fragElim);
	};
}
